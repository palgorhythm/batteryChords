...
const sleep = async (ms = 1000) => await new Promise((resolve) => setTimeout(resolve, 1000))
...
const handlers = {
  [Max.MESSAGE_TYPES.ALL]: (handled, ...args) => {
    ...
    const notesToPlay = Handler.handle({note: midiNote, velocity: midiVelocity, channel: midiChannel})
    const noteOns = notesToPlay.filter(np => np.velocity !== 0).map(np => np.note)
    if(noteOns.length){
