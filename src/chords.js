...
const _noteNameToMidi = ({ note, octaveMin, octaveMax }) => {
  ...
  const randomOctave = Math.round(
    octaveMin + Math.random() * (octaveMax - octaveMin)
  );
  ...
  for (let i = 0; i < noteNameChord.length; i++) {
    ...
    if (i === 0) {
      midi.push(
        _noteNameToMidi({
          note,
          octaveMin: CONFIG.bass.octave.min,
          octaveMax: CONFIG.bass.octave.max,
        })
      );
    } else {
      midi.push(
        _noteNameToMidi({
          note,
          octaveMin: CONFIG.chord.octave.min,
          octaveMax: CONFIG.chord.octave.max,
        })
      );
    }
  }
